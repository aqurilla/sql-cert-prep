...
-- SUBQUERY CAN BE USED AS A SOURCE TABLE

-- PAIRWISE COMPARISON SUBQ
SELECT * FROM EMPLOYEES
WHERE (MANAGER_ID, DEPARTMENT_ID) IN
  (SELECT MANAGER_ID, DEPARTMENT_ID FROM EMPLOYEES WHERE FIRST_NAME='John')
AND FIRST_NAME != 'John';

-- IF THE TWO CONDITIONS ARE INDEPENDENT, THEN CAN BE SPECIFIED SEPARATELY
SELECT * FROM EMPLOYEES
WHERE MANAGER_ID IN 
...
AND FIRST_NAME != 'John';
-- ABOVE QUERY CAN BE A COMBINATION OF ANY OF THE MANAGER IDS AND DEPT IDS

-- CORRELATED SUBQ REFERENCES PARENT COLUMN
...
-- IF NOT IN GETS NULL FOR ANY RECORD, IT RETURNS NULL 

-- ADVANTAGE OF WITH - CACHES RECORDS AND IS FASTER
WITH EMP
AS
(
...
VALUES (107, 'OP');
-- ERRORS OUT SINCE WHERE CONDITIONS ARE NOT MATCHED
...
SET SALARY = (SELECT SALARY FROM EMPLOYEES E WHERE E.EMPLOYEE_ID = E_COPY.EMPLOYEE_ID)
WHERE EXISTS (SELECT 1 FROM EMPLOYEES E WHERE E.EMPLOYEE_ID = E_COPY.EMPLOYEE_ID);
-- WHERE EXISTS CAN MAKE SURE THAT ONLY MATCHING RECORDS ARE UPDATED

DELETE FROM EMP_COPY
WHERE EXISTS (SELECT 1 FROM EMPLOYEES E WHERE E.EMPLOYEE_ID = EMP_COPY.EMPLOYEE_ID);
-- DELETES RECORDS WHICH ARE PRESENT IN PARENT TABLE
