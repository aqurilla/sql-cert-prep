...
-- TO CHANGE A COLUMN TO PRIMARY KEY
ALTER TABLE EMP2
MODIFY EMPLOYEE_ID PRIMARY KEY;
...
------------------------------------------------
-- METHODS ARE BY USING MODIFY, AND BY USING ADD CONSTRAINT
...
DELETE FROM DEPT2;
-- THROWS ERROR DUE TO REFERENTIAL INTEGRITY

ALTER TABLE DEPT2
DROP PRIMARY KEY CASCADE;
-- CASCADE HAS TO BE SPECIFIED FOR IT TO DROP ALL THE REFERENTIAL CONSTRAINTS
...
------------------------------------------------
-- DISABLING CONSTRAINTS MAY BE NEEDED DURING BIG DATA MIGRATIONS
-- ALSO IF THERE IS A REARRAGEMENT IN TABLE LOADING ORDER
...
ALTER TABLE EMP2
DISABLE CONSTRAINT EMP2_PK;
-- WHEN CONSTRAINT IS DISABLED, THE UNIQUE INDEX IS DROPPED

ALTER TABLE DEPT2
DISABLE CONSTRAINT DEPT2_PK CASCADE;
-- THIS COMMAND DISABLES BOTH THE PRIMARY KEY AND THE FOREIGN KEY CONSTRAINTS

-- DEFERRABLE CONSTRAINTS - ERROR CHECKING IS POSTPONED TILL COMMIT
...
-- DEFERRABLE INITIALLY IMMEDIATE SHOWS ERROR

SET CONSTRAINT SAL_CK IMMEDIATE;

-- A CONSTRAINT HAS TO BE MADE AS DEFERRABLE TO BE ABLE TO BE SET AS INITIALLY
-- IMMEDIATE OR DEFERRED

------------------------------------------------
-- GLOBAL TEMPORARY TABLE - ONLY CONTAINS DATA FOR CURRENT SESSION
...
RUN THIS IN CMD:
SQLLDR CONTROL=<PATH TO CTL FILE> [LOG=<PATH>]
*/
...
-- DML IS NOT SUPPORTED ON EXTERNAL TABLES

-- ORACLE DATAPUMP
CREATE TABLE EMP_PMP
  ( EMPLOYEE_NUMBER,
    FNAME,
    LNAME
  )
  ...
  ( TYPE ORACLE_DATAPUMP
    DEFAULT DIRECTORY EMP_DIR
    LOCATION ('FILEPATH.DMP')
  )
  AS
  SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  FROM EMPLOYEES;
-- THE SELECTED DATA IS STORED IN THE .DMP FILE, AND THEN USED TO CREATE THE
-- EXTERNAL TABLE EMP_PMP

-- WHEN DATA IS READ DIRECTLY FROM A .DMP FILE, THE DATATYPE MUST BE MENTIONED
CREATE TABLE EMP_PUMP_READ
  (
    EMPLOYEE_NUMBER NUMBER,
    FNAME VARCHAR2(100),
